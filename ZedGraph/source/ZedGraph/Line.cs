...
namespace ZedGraph
{
	...
	#region Rendering Methods
		...
		/// line segment in screen pixel units</param>
		public void DrawSegment( IGraphics g, GraphPane pane, float x1, float y1,
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		public void DrawSticks( IGraphics g, GraphPane pane, CurveItem curve, float scaleFactor )
		{
			Line source = this;
			if ( curve.IsSelected )
		...
		/// <param name="curve">A <see cref="LineItem"/> representing this
		/// curve.</param>
		public void DrawCurve( IGraphics g, GraphPane pane,
                                CurveItem curve, float scaleFactor, IPointList points)
		{
			Line source = this;
			if ( curve.IsSelected )
				source = Selection.Line;
						...
						else
						{
							...
							if ( !lastBad )
							{
								try
								{
									...
									{
										if ( !curve.IsSelected && ( this._gradientFill.IsGradientValueType || !(CustomDashes is NormalCustomDashStyle) ) )
										{
		...
		/// <param name="curve">A <see cref="LineItem"/> representing this
		/// curve.</param>
		public void DrawCurveOriginal( IGraphics g, GraphPane pane,
										  CurveItem curve, float scaleFactor )
		{
			Line source = this;
			if ( curve.IsSelected )
		...
		/// </summary>
		private void InterpolatePoint( IGraphics g, GraphPane pane, CurveItem curve, PointPair lastPt,
						...
						out PointF[] arrPoints, out int count )
		{
			arrPoints = null;
			count = 0;
			IPointList points = GetPointsForLowPointsArray(curve);
			...
			if ( this.IsVisible && !this.Color.IsEmpty && points != null )
			{
				...
				for ( int i = points.Count - 1; i >= 0; i-- )
				{
					...
					if ( !points[i].IsInvalid )
					{
                        if (this is FilledLine)
                        {
                            // FilledLines don't support stacking, so just get the points.  
                            x = points[i].X;
                            y = points[i].Y;
                        }
                        else
	    ...
	    protected virtual IPointList GetPointsForLowPointsArray(CurveItem curve)
	    {
	        return curve.Points;
	    }
		...
		public virtual void CloseCurve( GraphPane pane, CurveItem curve, PointF[] arrPoints,
