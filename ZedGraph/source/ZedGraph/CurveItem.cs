...
*/
	...
	#region Properties
		...
		public Color Color
		{
			get
			{
                ...
                else if (this is BubbleItem)
                    return ((BubbleItem) this).BubbleColor;
                else
                    return Color.Empty;
			}
			set 
			{
				...
				else if ( this is LineItem )
				{
                    ...
                    ((BubbleItem)this).BubbleColor = value;
            }
		}
	...
	#region Rendering Methods
		...
		/// </param>
		abstract public void Draw( IGraphics g, GraphPane pane, int pos, float scaleFactor  );
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		abstract public void DrawLegendKey( IGraphics g, GraphPane pane, RectangleF rect, float scaleFactor );
	...
	#region Utility Methods
										...
										GraphPane pane )
		{
		    ...
		    FilledLineItem filled = this as FilledLineItem;
			...
			for ( int i=0; i<this.Points.Count; i++ )
			{
			    ...
			    double curYLower = filled == null ? curY : filled.LowerPoints[i].Y;
				double curZ = isZOrdinal ? i + 1 : point.Z;

				bool outOfBounds = curX < xLBound || curX > xUBound ||
                    curY < yLBound || curY > yUBound || curYLower < yLBound || curYLower > yUBound || 
				...
				if (	ignoreInitial && curY != 0 &&
						( curY != PointPair.Missing || curYLower != PointPair.Missing ) )
					ignoreInitial = false;
						...
						curY != PointPair.Missing )
				{
                    ...
                    if (curYLower != PointPair.Missing)
                    {
                        if (curYLower < yMin)
                            yMin = curYLower;
                        if (curYLower > yMax)
