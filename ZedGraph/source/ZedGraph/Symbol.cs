...
namespace ZedGraph
{
	...
	#region Defaults
		...
		public struct Default
		{
            ...
            public static float GetPenWidth(SymbolType type)
            {
                // Choose the thickness of the pen to use based on the symbol type
                switch (type)
                {
                    case SymbolType.Plus:
                        return UltraThickPenWidth;
                    case SymbolType.XCross:
                    case SymbolType.HDash:
                    case SymbolType.VDash:
                        return ThickPenWidth;
                    default:
                        return PenWidth;
                }
            }
            ...
            /// </summary>
            private const float PenWidth = 1.0F;
            /// <summary>
            /// The default pen width to be used for drawing curve symbols which need to be thick
            /// (<see cref="ZedGraph.LineBase.Width"/> property).  Units are points.
            /// </summary>
            private const float ThickPenWidth = 2.0F;
            /// <summary>
            /// The default pen width to be used for drawing curve symbols which need to be thick
            /// (<see cref="ZedGraph.LineBase.Width"/> property).  Units are points.
            /// </summary>
            private const float UltraThickPenWidth = 3.0F;
			/// <summary>
	...
	#region Properties
		...
		public SymbolType Type
		{
			...
			set
			{
                ...
                // Check that the border/pen width is still valid
			    _border.Width = Default.GetPenWidth(_type);
			}
		}
		/// <summary>
	...
	#region Constructors
		...
		public Symbol( SymbolType type, Color color )
		{
            ...
            _border = new Border(Default.IsBorderVisible, color, Default.GetPenWidth(type));
	...
	#region Rendering Methods
		...
		/// </param>
		public void DrawSymbol( IGraphics g, GraphPane pane, int x, int y,
		...
		public GraphicsPath MakePath( IGraphics g, float scaleFactor )
		{
            ...
            switch (_type == SymbolType.Default || (_type == SymbolType.UserDefined && _userSymbol == null) ? Default.Type : _type)
            {
                ...
                case SymbolType.Star:
                    ...
                    path.AddLine(-hsize, 0, hsize, 0);
                    path.StartFigure();
                    path.AddLine(-hsize1, -hsize1, hsize1, hsize1);
                    path.StartFigure();
                    path.AddLine(hsize1, -hsize1, -hsize1, hsize1);
                    break;
