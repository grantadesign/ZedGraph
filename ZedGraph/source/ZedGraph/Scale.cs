...
namespace ZedGraph
{
	...
	#region Fields
	    ...
	    protected int _minimumNumberOfMajorTicks;
	    #endregion
...
*/
	    ...
	    /// <summary>
	    /// <b>Only applies to log scales</b>
	    /// Gets or sets the minimum number of Major tick marks on the axis before it changes its tick mark positioning
	    /// regime to that of a linear scale.  Set to 0 to -ve number to have the log scale never use linearly spaced tick marks -
	    /// this may mean that axis will have no major tick marks and thus no labels though!
	    /// </summary>
	    public virtual int MinimumNumberOfMajorTicks
	    {
	        get { return _minimumNumberOfMajorTicks; }
	        set {_minimumNumberOfMajorTicks = value; }
	    }
	    /// <summary>
	    /// Returns true if the log scale is using tick marks which are spaced linearly (i.e. the major ticks are not at
	    /// 1, 10, 1000,... but are at values such as 400, 500, 600, 700 ).
	    /// This property can only be true if <see cref="MinimumNumberOfMajorTicks"/> is greater than 0 and the minimum and maximum
	    /// do not span more than <see cref="MinimumNumberOfMajorTicks"/>.
	    /// </summary>
	    /// <remarks>
	    /// This property returns true for all scale types apart from <see cref="ExponentScale"/> (which always returns false)
	    /// and <see cref="LogScale"/> which may return true if <see cref="MinimumNumberOfMajorTicks"/> was set to a value &gt; 0 and the data
	    /// range did not span that many powers of ten.
	    /// </remarks>
	    public virtual bool UsingLinearlySpacedTickMarks {get{return true;} }
	    #endregion
/*
	#region events
...
*/
		...
		internal SizeF GetScaleMaxSpace( IGraphics g, GraphPane pane, float scaleFactor,
							bool applyAngle )
		{
			if ( _isVisible )
			{
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		internal void DrawLabels( IGraphics g, GraphPane pane, double baseVal, int nTics,
...
//					grid.Draw( g, gridPen, pixVal2, topPix );
		...
		internal void DrawGrid( IGraphics g, GraphPane pane, double baseVal, float topPix, float scaleFactor )
		{
			MajorTic tic = _ownerAxis._majorTic;
			MajorGrid grid = _ownerAxis._majorGrid;
...
//				float lastPixVal = -10000;
		...
		internal void DrawLabel( IGraphics g, GraphPane pane, int i, double dVal, float pixVal,
		...
		/// </param>
		internal void Draw( IGraphics g, GraphPane pane, float scaleFactor, float shiftPos )
		{
...
//			_ownerAxis.DrawMinorTics( g, pane, baseVal, shiftPos, scaleFactor, topPix );
	...
	#region Scale Picker Methods
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		virtual public void PickScale( GraphPane pane, IGraphics g, float scaleFactor )
		{
		...
		/// font sizes, etc. according to the actual size of the graph.
		/// </param>
		public int CalcMaxLabels( IGraphics g, GraphPane pane, float scaleFactor )
		{
